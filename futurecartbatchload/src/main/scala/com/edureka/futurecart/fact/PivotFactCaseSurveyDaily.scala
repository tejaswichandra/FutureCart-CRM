...
object PivotFactCaseSurveyDaily extends FactSparkApp {
  ...
  def executeEtlProcess(spark: SparkSession): Unit = {
    ...
    val pivot_df = fact_case_srvy_df.join(dim_case_catg_df,
      ...
      .select(fact_case_srvy_df("case_no"), col("create_timestamp"), col("last_modified_timestamp"), col("status"),
        col("communication_mode"), col("case_close_date"), col("survey_date"), col("case_create_date"),
        dim_case_catg_df("priority"), dim_case_priority_df("priority").as("priority_details"), dim_case_priority_df("severity"),
        dim_case_priority_df("sla").as("sla_hours"), col("emp_df.first_name").as("employe_first_name"),
        col("emp_df.last_name").as("employe_last_name"), col("manager_df.first_name").as("manager_first_name"),
        col("manager_df.last_name").as("manager_last_name"), dim_product_df("department"),
        dim_product_df("brand"), dim_product_df("commodity_desc"), dim_product_df("sub_commodity_desc"),
        col("cal_open.week_name").as("case_open_week"), col("cal_open.month_name").as("case_open_month"),
        col("cal_open.quarter_name").as("case_open_quarter"), col("cal_close.week_name").as("case_close_week"),
        col("cal_close.month_name").as("case_close_month"), col("cal_close.quarter_name").as("case_close_quarter"),
        dim_call_center_df("call_center_vendor"), col("con2.name").as("call_center_country"), col("con1.name").as("case_country"),
        fact_case_srvy_df("Q1"), fact_case_srvy_df("Q2"), fact_case_srvy_df("Q3"), fact_case_srvy_df("Q4"), fact_case_srvy_df("Q5"),
        col("q1_df.negative_lower").as("q1_negative_lower"), col("q1_df.negative_upper").as("q1_negative_upper"),
        col("q1_df.neutral_lower").as("q1_neutral_lower"),col("q1_df.neutral_upper").as("q1_neutral_upper"),
        col("q1_df.positive_lower").as("q1_positive_lower"),col("q1_df.positive_upper").as("q1_positive_upper"),
        col("q2_df.negative_lower").as("q2_negative_lower"),col("q2_df.negative_upper").as("q2_negative_upper"),
        col("q2_df.neutral_lower").as("q2_neutral_lower"),col("q2_df.neutral_upper").as("q2_neutral_upper"),
        col("q2_df.positive_lower").as("q2_positive_lower"),col("q2_df.positive_upper").as("q2_positive_upper"),
        col("q3_df.negative_lower").as("q3_negative_lower"),col("q3_df.negative_upper").as("q3_negative_upper"),
        col("q3_df.neutral_lower").as("q3_neutral_lower"),col("q3_df.neutral_upper").as("q3_neutral_upper"),
        col("q3_df.positive_lower").as("q3_positive_lower"),col("q3_df.positive_upper").as("q3_positive_upper"),
        col("q5_df.negative_lower").as("q5_negative_lower"),col("q5_df.negative_upper").as("q5_negative_upper"),
        col("q5_df.neutral_lower").as("q5_neutral_lower"),col("q5_df.neutral_upper").as("q5_neutral_upper"),
        col("q5_df.positive_lower").as("q5_positive_lower"), col("q5_df.positive_upper").as("q5_positive_upper"))

    val pivot_final_df = pivot_df.select(col("case_no"),
    ...
    pivot_final_df.write.format("orc").mode(SaveMode.Overwrite).saveAsTable("futurecart_dw.pivot_fact_futurecart_case_survey_dly")
  }
